...
const PROMPT_VERSION = 'v2_query_only_scaffold';

function buildSystemPrompt(): string {
  return `You are a rule induction agent. Available rules: ${DISTINCT_RULES.join(', ')}.

You have at most 6 queries. Each query must be accompanied by structured reasoning.

OUTPUT ONLY ONE LINE OF JSON PER TURN.
- Query: {"top_2_hypotheses":["RULE_ID1","RULE_ID2"],"why_discriminative":"Brief reason","action":"query","x":[x0,x1,x2]}
...
function parseQueryOnlyResponse(text: string): { action: 'query' | 'final' | null; x: [number, number, number] | null; ruleId: string | null } {
...
export async function runQueryOnlyScaffoldBatch(tasks: Task[], output: OutputManager, api: ApiClientWrapper, model: string = 'deepseek-chat'): Promise<void> {
  for (let i = 0; i < tasks.length; i++) {
    ...
    for (let step = 0; step < 14; step++) {
      ...
      const parsed = parseQueryOnlyResponse(text);
...
if (process.argv[1] && process.argv[1].endsWith('runQueryOnlyScaffold.ts')) {
  ...
  output.log(`Running query_only_scaffold batch: ${tasksData.length} tasks, model=${opt.model}`);
  await runQueryOnlyScaffoldBatch(tasksData, output, api, opt.model);
  output.close();
}
