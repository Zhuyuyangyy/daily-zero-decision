...
├── domain/             ← Domain 层（新增）
│   ├── achievements.ts ← 已在 utils/，迁入并加测试
│   └── streak.ts       ← 从 storage.ts 拆出纯函数
...
// ... SettingsRepository, MoodRepository, HistoryRepository
```
...
- [ ] 3 个 Repository 接口 + Dexie 实现 + fake
- [ ] 5 个 hook 迁移到 repository（useTasks / usePet / useStreak / useAppState / usePeace）
